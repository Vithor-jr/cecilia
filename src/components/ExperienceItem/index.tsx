...
export default function ExperienceItem ({data}:ExperienceProps) {
	return(
		<div className={`${poppins.className} flex flex-row gap-2 items-center`}>
			...
			<div>
				<h1 className="font-bold text-[18px] text-[#6D6D6D]">{data.title}</h1>
				<div className="flex flex-row gap-2">
					<p className="flex flex-row gap-1 font-medium text-[12px] text-[#B5B5B5]">{data.date}</p>
					<p className="flex flex-row gap-1 font-medium text-[12px] text-[#858585]">{data.prof}</p>
				</div>
				<p className="text-[#858585] text-[12px]">{data.labels}</p>	
			</div>
		</div>
	)
}